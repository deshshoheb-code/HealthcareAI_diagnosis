...
def detect_intent(query: str) -> str:
    ...
    if any(x in q for x in [
        ...
        "presentation", "signs", "symptoms", "evaluation"
    ]):
        return "diagnosis"
...
def build_disease_chapter_filter(
    ...
    - ANY of the target chapter numbers
    OR
    - disease_name contains disease term
    OR
    - chapter_title contains disease term
    """

    filters = None

    # OR together all chapter filters
...
def post_filter_results(results: List[Dict]) -> List[Dict]:
    ...
    for r in results:
        key = (
            ...
            (r.get("chapter_title") or "").strip().lower(),
            (r.get("subsection_title") or "").strip().lower(),
            (r.get("content") or "")[:150].strip().lower(),
        )
...
def preference_score(
    ...
    keyword_bonus = 0
    if intent == "diagnosis":
        keyword_bonus = sum(term in content for term in diagnosis_terms)
    elif intent == "treatment":
        keyword_bonus = sum(term in content for term in treatment_terms)
    elif intent == "investigation":
        keyword_bonus = sum(term in content for term in investigation_terms)
